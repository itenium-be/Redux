...
const _initialFilters: IProvidersFilter = {
    search: '',
    filters: {
        status: {
            value: 0,
            label: 'enums.providerStatus.all'
        }
    }
};
...
export const initialFilters = (): IProvidersFilter => JSON.parse(JSON.stringify(_initialFilters));
...
export function reducer(state: ProvidersState = INITIAL_STATE, action: ProviderActions.Actions): ProvidersState {
    switch (action.type) {
        ...
        case ProviderActions.LOAD_PROVIDERS_FAIL: {
            return Object.assign({}, state, { loading: false, loaded: false, providers: [] });
        }
            ...
            return {
                ...
                filters: {
                    ...
                    filters: {
                        ...
                        status: payload.filters.status.value !== -1 ? payload.filters.status : initialFilters().filters.status
                    }
                }
            };
        }
        case ProviderActions.SET_SORTING: {
            ...
            sorting.sortField = payload.sortField ? payload.sortField : initialSorting().sortField;
            sorting.sortOrder = payload.sortOrder ? payload.sortOrder : initialSorting().sortOrder;
        ...
        case ProviderActions.SET_PAGING: {
            ...
            paging.pageSize = payload.pageSize ? payload.pageSize : initialPaging().pageSize;
            return Object.assign({}, state, { paging: paging });
        }
        case ProviderActions.RESET_PAGE: {
            return Object.assign({}, state, { paging: initialPaging(), sorting: initialSorting(), filters: initialFilters() });
        }
